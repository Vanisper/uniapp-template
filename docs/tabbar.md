...
`src/configs/theme.ts` 中的 `tabbar.mode` 控制原生或自定义底栏，`tabbar.height` 控制自定义底栏的完整视觉高度。

| 配置 | 值 | 行为 |
| --- | --- | --- |
| `tabbar.mode` | `default` | 使用平台底栏，页面布局不渲染 `AppPageTabbar` |
| `tabbar.mode` | `custom` | 使用页面内底栏，由接入层处理原生底栏隐藏 |
| `tabbar.height` | 数字 | 自定义底栏的完整视觉高度，单位 px，不包含底部安全区 |

`pages.config.ts` 同样读取模式配置来生成平台设置，修改模式后应重新启动开发服务或重新构建。平台专属字段优先放在框架支持的平台节点中，其余字段按 `UNI_PLATFORM` 筛选。该文件在 Node 中执行，`#ifdef` 注释不会参与条件编译。

高度包含组件的抬升区域，当前配置为 72px。浮起装饰可以向前覆盖内容区边缘，避免为装饰留出贯穿整行的空白；滚动内容应保留足够的尾部留白，让最后一项可以完整移出覆盖区。底部安全区仍由 `PageWrapper` 处理。

覆盖区域的视觉与点击范围应分开处理。透明区域允许点击和滚动穿透到内容，标签命中区从底栏表面起算；图文装饰不应截取底层标签的点击。
...
与 UnoCSS 属性模式重名的组件参数应先在脚本中定义，再通过变量绑定，如 `:color="colors.normal"`。静态 `color`、`active-color` 可能被转换成类名；直接绑定颜色字面量也可能被提取器误识别为属性选择器。应同时验证组件收到的值和最终生成的 CSS。
...
作用域插槽还可能生成自动高度的中间容器。绝对定位的图文应根据组件高度建立明确的定位区域，不能仅靠多层 `height: 100%` 传递尺寸。
...
接入层仅为页面容器已经预留的底部安全区补齐背景，不再次占位。替换底栏表面颜色时，需要同步这一背景，使手势区域与底栏衔接一致。
