  ...
  <view class="user-center-header fixed top-0 left-0 box-border w-full h-480 p-x-3">
    ...
    <block v-if="currAuthStep === 1">
      <view class="header" @click="gotoUserEditPage">
        <wd-img width="96rpx" height="96rpx" round :src="userInfo.avatarUrl || defaultAvatarUrl" />
    ...
    <block v-if="currAuthStep === 2">
      <view class="header">
        ...
        <!-- 需要授权用户信息，通过slot添加弹窗 -->
        <view v-if="isNeedGetUserInfo" class="header__transparent">
          <slot name="getUserInfo" />
        </view>
        <!-- 不需要授权用户信息，仍然触发gotoUserEditPage事件 -->
        <view v-else class="header__transparent" @click="gotoUserEditPage" />
      </view>
    </block>
    <!-- 已登录且已经授权用户信息的情况 -->
